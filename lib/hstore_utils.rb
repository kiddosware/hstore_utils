      ...
      define_method(key) do
        if send(hstore_field)
          ...
          if (value.class.to_s == "String")
            #hstore stores values as strings but in this case we have an array as a string, so we have to convert it (without using eval!)
            return value[1..-2].split(',').map {|s| s.to_i}
          end
          return value
        end
      end
